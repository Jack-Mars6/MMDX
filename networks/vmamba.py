...
def channel_shuffle(x: Tensor, groups: int) -> Tensor:
    ...
    # reshape将num_channels的数据划分为groups个组
    # [batch_size, height, width, num_channels] -> [batch_size, height, width, groups, channels_per_group]
    x = x.view(batch_size, height, width, groups, channels_per_group)

    # contiguous：将Tensor数据转化为内存中连续的数据
...
class Conv_VSSBlock(nn.Module):
    ...
    def forward(self, input: torch.Tensor):
        # input_left, input_right = input.chunk(2, dim=-1)    # (b, h, w, c/2)
        input_left, input_right = input.clone(), input.clone()
        ...
        output = torch.add(input_left, x)
        output = self.finalconv11(output.permute(0, 3, 1, 2).contiguous()).permute(0, 2, 3, 1).contiguous()
        return output + input

class CA_VSSBlock(nn.Module):
    def __init__(
            self,
            hidden_dim: int = 0,
            drop_path: float = 0,
...
class VSSLayer_up_ca(nn.Module):
    """ A basic Swin Transformer layer for one stage.
    Args:
        dim (int): Number of input channels.
        depth (int): Number of blocks.
...
class VSSLayer_up_conv(nn.Module):
    """ A basic Swin Transformer layer for one stage.
    Args:
        dim (int): Number of input channels.
        depth (int): Number of blocks.
...
class VSSLayer_up_base(nn.Module):
    """ A basic Swin Transformer layer for one stage.
    Args:
        dim (int): Number of input channels.
        depth (int): Number of blocks.
...
class VSSM_conv(nn.Module):
    def __init__(self, patch_size=4, in_chans=3, num_classes=2, depths=[2, 2, 9, 2], depths_decoder=[2, 9, 2, 2],
